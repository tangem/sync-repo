...
public class DeployHeaderSerialization {
    ...
    public static func serialize(from: DeployHeader) -> String {
        var retStr =  from.account +  CLTypeSerializeHelper.uInt64Serialize(input: Utils.dateStrToMilisecond(dateStr: from.timestamp)) + CLTypeSerializeHelper.uInt64Serialize(input: Utils.ttlToMilisecond(ttl: from.ttl)) + CLTypeSerializeHelper.uInt64Serialize(input: from.gasPrice) + from.bodyHash
